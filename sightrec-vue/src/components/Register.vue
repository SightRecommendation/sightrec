<template>
  <div class="register_container"
       :style="{'background': 'linear-gradient(to left bottom, hsl(' + leftColor + ', 100%, 85%) 0%,hsl(' + bottomColor + ', 100%, 85%) 100%)'}">
    <div class="register_box">
      ...
      <el-form ref="registerFormRef"
        ...
        <el-form-item prop="name">
          <el-input placeholder="请输入长度为 3 - 15 的用户名"
  ...
  methods: {
    ...
    register () {
      this.$refs.registerFormRef.validate(async valid => {
        if (!valid) return
        if (this.registerForm.password !== this.registerForm.checkPassword) { this.$message.error('两次输入密码不一致！'); return }
