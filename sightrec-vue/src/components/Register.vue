  ...
  methods: {
    ...
    login () {
      this.$refs.loginFormRef.validate(async valid => {
        if (!valid) return
        if (this.loginForm.password !== this.loginForm.password1) { alert('两次输入密码不一致！'); return }
        const { data: result } = await this.$http.post('/users/register', this.loginForm)
        // const response = await this.$http.post('/admins/login', this.loginForm)
        if (result.meta.status !== 200) this.$message.error('注册失败')
